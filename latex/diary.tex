...
\begin{enumerate}
	\item made 227x227 data for alexnet
	\item found dobro detail: they don't actually reconstruct image dimensions of 227x227, but smaller reconstructions of 128x128 and 192x192.
	\item enabled shared filters on separable priors
\end{enumerate}

\section*{September 10.}
\begin{enumerate}
	\item some extra research on MRF / Hammersley-Clifford: seems like, assuming the clique potentials are the accurate model (which they are not, are they?), no additional weighting is necessary.
	\item implementing build of separable priors, verifying norming and whitening application
	\item half day
\end{enumerate}

\section*{September 9.}
\begin{enumerate}
	\item redoing norming of weight matrixes
	\item refactoring
	\item changing to channelwise whitening on separable priors
\end{enumerate}

\section*{September 8.}
\begin{enumerate}
	\item compiling results
	\item meeting
	\item half day
\end{enumerate}
...
\section*{September 7.}
\begin{enumerate}
	\item Vgg Score implementation
	\item student dist for channelwise priors
\end{enumerate}

\section*{September 6.}
\begin{enumerate}
	\item tested with PCA, ZCA and unwhitened image data
	\item tested ZCA on full prior
	\item started Vgg score implementation
\end{enumerate}

\section*{September 5.}
\begin{enumerate}
	\item implemented separable separable prior training
	\item validated ICA filter derivation
		
\end{enumerate}

\section*{September 4.}
\begin{enumerate}
	\item Starting to implement separable prior - working out derivation of implicit ICA filters from convolutional ones
\end{enumerate}

\section*{September 3.}
\begin{enumerate}
	\item half day: 
	\item refactoring FoE priors so distributions are an argument rather than different classes. a lot of cleaning up and re-naming for clarity, ensuring backward compatibility
	\item reading up in separable convolutions
\end{enumerate}


\section*{September 2.}
\begin{enumerate}
	\item modular inverting nets updated and tested
	\item updated inference on full and channel-wise foe priors to use convolutions more efficiently if pre-processing allows
\end{enumerate}
