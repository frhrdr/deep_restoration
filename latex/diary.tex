...
\begin{enumerate}
	\item Week goal 1: send out summary + timeline out to potential examiners before friday.
	\item Week goal 2: find out, why first ICA prior results are so bad.
	...
	\subitem give more details on result setups
	\subitem draft related works (so the big names are connected)
	\item do search on samplings from FoE models
	\item trained first batch of foe priors (looking very similar to ica priors.)
	\item train ica prior with more (and fewer?) components
	\item train ica prior reconstruction with different weightings
	\item train ica prior reconstruction with l-bfgs
	\item test ica prior on vgg16 in addition to alexnet
\end{enumerate}
